...
@interface HZTLaunchAdManager ()<XHLaunchAdDelegate>

@end

@implementation HZTLaunchAdManager

+(void)load{
    [self shareManager];
}

+(HZTLaunchAdManager *)shareManager{
    static HZTLaunchAdManager * instance = nil;
    static dispatch_once_t oneToken;
    dispatch_once(&oneToken,^{
        instance = [[HZTLaunchAdManager alloc] init];
    });
    return instance;
}
...
-(void)example01{
    ...
    //广告数据请求
//    [[TCHomeNetWorkManager manager]getLaunchAdImageDataSuccessd:^(id responseObject) {
//        NSLog(@"广告数据 = %@",responseObject);
//        //广告数据转模型
//        TCLaunchAdModel *model = [[TCLaunchAdModel alloc] initWithDict:responseObject[@"data"]];
//        //配置广告数据
...
-(void)example02{
    ...
    //广告frame
    imageAdconfiguration.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);// [UIScreen mainScreen].bounds.size.width/1242*1786;
    //广告图片URLString/或本地图片名(.jpg/.gif请带上后缀)
