...
//        progressView.progressTintColor = TCColor(153, 153, 153);  //进度条的颜色
//        progressView.trackTintColor = TCColor(220 , 220, 220);        //进度条背景颜色
        if (iOS7gt) {
...
- (void)setPhoto:(ZLPhotoPickerBrowserPhoto *)photo{
    ...
    if (photo.photoURL.absoluteString.length) {
        ...
        if (photoRange.location != NSNotFound){
            ...
            // 网络URL
            [_photoImageView sd_setImageWithURL:photo.photoURL placeholderImage:ToolGetImageWithColor(RGBColor(216, 216, 216)) options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
