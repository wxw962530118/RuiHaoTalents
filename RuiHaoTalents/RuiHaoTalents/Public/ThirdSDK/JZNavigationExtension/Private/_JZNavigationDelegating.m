...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if (![navigationController.delegate isEqual:navigationController.jz_navigationDelegate] && ![navigationController.delegate isKindOfClass:NSClassFromString(@"VCInteractionDelegate")]) {
        /***
         处理  因为 和 转场动画 HHTransition里面的 VCInteractionDelegate 协议方法有冲突 导致的闪退
         [_JZNavigationDelegating navigationController:willShowViewController:animated:]: unrecognized selector sent to instance 0x600000237720
         */
